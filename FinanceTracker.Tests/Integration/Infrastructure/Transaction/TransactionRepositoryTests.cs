...
public sealed class TransactionRepositoryTests : DatabaseFixture
{
	...
	private static Core.Domains.Transaction.Transaction CreateTransaction()
	{
		return Core.Domains.Transaction.Transaction.Create(
	...
	{
		Core.Domains.Transaction.Transaction? result = await _transactionRepository.GetByIdAsync(transactionId: Guid.NewGuid());
	...
	public async Task SaveAsync_ThenGetByIdAsync_ShouldRestoreTransaction()
	{
		...
		Core.Domains.Transaction.Transaction? loaded = await _transactionRepository.GetByIdAsync(transactionId: transaction.Id);
	...
	public async Task SaveAsync_WithMultipleEvents_ShouldRestoreCorrectState()
	{
		...
		Core.Domains.Transaction.Transaction? final = await _transactionRepository.GetByIdAsync(transactionId: transaction.Id);
