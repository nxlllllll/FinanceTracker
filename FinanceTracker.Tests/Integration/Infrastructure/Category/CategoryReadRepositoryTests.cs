...
public sealed class CategoryReadRepositoryTests : DatabaseFixture
{
    ...
    {
        Core.Domains.Category.Category? result = await _readRepository.GetByIdAsync(categoryId: Guid.NewGuid());
    ...
    {
        IReadOnlyList<Core.Domains.Category.Category> result = await _readRepository.GetAllAsync(userId: Guid.NewGuid());
    ...
    public async Task GetAllAsync_ShouldReturnOnlyUserCategories()
    {
        ...
        await CreateAndSaveCategoryAsync(userId: Guid.NewGuid());
