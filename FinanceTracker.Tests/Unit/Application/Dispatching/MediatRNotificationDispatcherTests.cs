...
public sealed class MediatRNotificationDispatcherTests
{
    ...
    [Test]
    public async Task DispatchAsync_WithTransactionAggregateType_ShouldPublishTransactionEventsNotification()
    {
        AggregateNotification notification = new AggregateNotification(
            AggregateId: Guid.NewGuid(),
            AggregateType: nameof(Transaction),
            Events: [new TransactionExcluded(
                Id: Guid.NewGuid(),
        ...
        await _publisher.Received(requiredNumberOfCalls: 1).Publish(
            notification: Arg.Is<TransactionEventsNotification>(predicate: n =>
                n.TransactionId == notification.AggregateId &&
                n.Events.Count == 1
            ),
            cancellationToken: Arg.Any<CancellationToken>()
        );
    ...
    [Test]
    public async Task DispatchAsync_WithUnknownAggregateType_ShouldThrowInvalidOperationException()
    {
        AggregateNotification notification = new AggregateNotification(
            AggregateId: Guid.NewGuid(),
