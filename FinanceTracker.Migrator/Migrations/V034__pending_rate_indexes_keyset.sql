...
create index idx_rm_transactions_pending_rate on rm_transactions (occurred_at, id)
    include (account_id, amount, currency_code, base_currency_code, exchange_rate, direction_type, row_version)
...
create index idx_rm_transfers_pending_rate on rm_transfers (occurred_at, id)
    include (from_account_id, to_account_id, amount_from, currency_from, currency_to, exchange_rate, row_version)
    where is_rate_pending = true;
